...
def main():
    ...
    with open("index.html", "w", encoding="utf-8") as f:
        ...
        f.write("body { font-family: system-ui, sans-serif; background: #f0f2f5; margin: 0; padding: 10px; color: #1c1e21; }")
        ...
        f.write(".filters-group { margin-top: 12px; border-top: 1px solid #eee; padding-top: 10px; }")
        f.write(".label { font-size: 0.75rem; font-weight: bold; color: #65676b; display: block; margin-bottom: 5px; text-transform: uppercase; }")
        ...
        f.write(".meta { font-size: 0.75rem; color: #1a73e8; font-weight: bold; margin-bottom: 10px; display: flex; justify-content: space-between; }")
        f.write("a { text-decoration: none; color: #000; font-weight: 800; display: block; font-size: 1.2rem; line-height: 1.3; margin-bottom: 8px; }")
        f.write(".summary { font-size: 0.95rem; color: #444; background: #f8f9fa; padding: 10px; border-radius: 6px; margin-bottom: 10px; }")
        f.write(".orig { font-size: 0.7rem; color: #999; font-style: italic; }")
        f.write("#search { width: 100%; padding: 12px; border-radius: 8px; border: 1px solid #ddd; box-sizing: border-box; }")
        ...
        # Choix de la portée de recherche
        ...
        f.write("<button class='btn btn-scope' onclick='setScope(\"title\", this)'>TITRE UNIQUEMENT</button>")
        f.write("<button class='btn btn-scope' onclick='setScope(\"desc\", this)'>RÉSUMÉ UNIQUEMENT</button></div>")
        ...
        f.write("arts.forEach(a=>{ const t=a.getAttribute('data-title').toLowerCase(); const d=a.getAttribute('data-desc').toLowerCase();")
        f.write("const s=a.getAttribute('data-src'); const date=a.getAttribute('data-date');")
        
        # Logique de recherche ciblée
        f.write("let matchQ = false; if(scope==='all') matchQ = t.includes(q) || d.includes(q); else if(scope==='title') matchQ = t.includes(q); else matchQ = d.includes(q);")
