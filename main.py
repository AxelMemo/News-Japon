...
def generate_html(data):
    ...
    # Construction des boutons de catégories
    ...
    # Template HTML pur (sans f-string pour éviter les erreurs de syntaxe)
...
<head>
    ...
    <style>
        ...
        .update-date { font-size: 0.8rem; color: #888; margin-bottom: 15px; }
...
<body>
    <div class="header">
        <h1>🇯🇵 Veille Japon</h1>
        <div class="update-date">Mise à jour : [[DATE]]</div>
        ...
        function filter() {
            ...
            articles.forEach(a => {
                ...
                if (visible && smartMode) {
                    let dupe = seen.find(s => getSim(s.title, t) > 0.65);
                    if (dupe) { 
                        visible = false; 
    ...
    final_html = template.replace("[[DATE]]", now)
    final_html = final_html.replace("[[BUTTONS]]", buttons_html)
    final_html = final_html.replace("[[ARTICLES]]", articles_list_html)
